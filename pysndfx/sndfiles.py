...
ENCODINGS_MAPPING = {np.int16: "s16", np.float32: "f32", np.float64: "f64"}
...
class FilePathInput(SoxInput):
    def __init__(self, filepath):
        ...
        stdout, stderr = Popen(
            shlex.split(info_cmd, posix=False), stdout=PIPE,
...
class FileBufferInput(SoxInput):
    def __init__(self, fp):
        ...
        wave_file = wave.open(
            fp, mode="rb")  # wave.open() seems to support only 16bit encodings
        self.channels = wave_file.getnchannels()
        self.data = np.frombuffer(
            wave_file.readframes(wave_file.getnframes()), dtype=np.int16)
...
class NumpyArrayInput(SoxInput):
    def __init__(self, snd_array, rate):
        ...
        self.cmd_prefix = ' '.join([
            "-t " + ENCODINGS_MAPPING[snd_array.dtype.type], "-r " + str(rate),
            "-c " + str(self.channels), PIPE_CHAR
        ])
